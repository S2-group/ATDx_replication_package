...
def pad_with_zero_projects(projects_with_issues, all_projects_path, organization, ar_rules):
    to_merge = {}
    all_projects = json.load(open(all_projects_path, 'r'))
    for p in all_projects:
        if(p['organization'] == organization and (not p['key'] in projects_with_issues)):
            to_merge[p['key']] = {
                    'projectKey':  p['key'],
                    'design_issues': 0
                } 
...
def count_ar_issues(arch_issues, organization, all_projects_path, ar_rules, ar_issues_path):
    counted_ar_issues = {}
    for i in arch_issues:
        if arch_issues[i]['organization'] == organization:
            if not arch_issues[i]['project'] in counted_ar_issues:
                ...
                counted_ar_issues[arch_issues[i]['project']][arch_issues[i]['rule']] += 1
    counted_ar_issues = pad_with_zero_projects(counted_ar_issues, all_projects_path, organization, ar_rules)
...
if __name__ == "__main__":
    ...
    # count_ar_issues(arch_issues, 'onap', './data/merged_projects.json', ar_rules, './data/onap_ar_issues.csv')
